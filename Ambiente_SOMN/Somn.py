...
class Somn(Env):
    ...
    def __init__(self, env_config=None
        ...
        Somn.objetivo = env_config.get('objetivo', 0)
        ...
        self.match = np.zeros(env_config.get('N', 10))

        self.M = env_config.get('M', 10)
        self.N = env_config.get('N', 10)
        self.Y = env_config.get('Y', 10)
        self.MAXDO = env_config.get('MAXDO', 100)
        self.MAXAM = env_config.get('MAXAM', 2)
        self.MAXPR = env_config.get('MAXPR', 2)
        self.MAXPE = env_config.get('MAXPE', 10)
        self.MAXFT = env_config.get('MAXFT', 5)
        self.MAXMT = env_config.get('MAXMT', 3)
        self.MAXTI = env_config.get('MAXTI', 2)
        self.MAXEU = env_config.get('MAXEU', 5)
        self.atraso = env_config.get('atraso', -1)
        ...
        #self.seed = seed
        self.atraso = self.atraso # (by_frederic)
        ...
        self.ub_MT = np.array([self.MAXFT-1 for _ in range(self.M)]).astype(np.int64)
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(323,), dtype=np.float64)
    ...
    def store(self, i: int):
        if self.DE[i].ST == 4:
            ...
            self.totPenalty += self.lucro * tx_ambiente * 0.01
    ...
    def reject(self, i: int):
        if self.DE[i].ST == 2:
            ...
            tx_ambiente = self.DE[i].err
            self.lucro = self.DE[i].PR - self.DE[i].CO
            self.totPenalty += self.lucro * tx_ambiente * 0.0005
    ...
    def step(self, action):
        ...
        # Transformar observação
        flattened_obs = self.flatten_obs(observation)
        # Limitar os valores da observação para o intervalo [0, 1]
        clipped_observation = np.clip(flattened_obs, 0, 1)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        return (clipped_observation, info)  # by_frederic: para se adequar ao Gymnasium
    
    # Função para achatar e concatenar a observação
